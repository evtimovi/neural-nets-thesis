...
with open('howfar_vgg_printcodes_subject_00070_with_imposter.csv', 'w') as f:
...
#    true_meb=stom[subject]
        ...
        for c in mebs_fa:
            f.write(str(subject) + ',fa,' + ','.join(map(str,c))+"\n")
        for c in mebs_fb:
            f.write(str(subject) + ',fb,' + ','.join(map(str,c))+"\n")
        for c in mebs_rc:
            f.write(str(subject) + ',rc,' + ','.join(map(str,c))+"\n")
        for c in mebs_fa_imposter:
            f.write(str(subject) + ',fa,' + ','.join(map(str,c))+"\n")
        for c in mebs_fb_imposter:
            f.write(str(subject) + ',fb,' + ','.join(map(str,c))+"\n")
        for c in mebs_rc_imposter:
            f.write(str(subject) + ',rc,' + ','.join(map(str,c))+"\n")
